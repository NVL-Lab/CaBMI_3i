...
def BMI_Acqnvs_3i(path_data, expt_str, baselineCalibrationFile, tset, vector_stim, debug_bool, debug_input, base_val_seed, fb_bool, fb_cal, a) -> None:

    with open('expt2bmi_flag.json', 'r') as file:
        flag_settings = json.load(file)

    # Load flag configuration file
    flagBMI, flagDRstim, flagStimRandom, flagWater = flag_settings.get(expt_str, None)
    ...
    #bdata = load(fullfile(baseline_calib_file))
    ...
    # Cleaning 
    #finishup = onCleanup(@() cleanMeUp(path_data['save_path'], bdata, debug_bool));
    ...
    if not debug_bool:
        ...
        savePrairieFiles(path_data["save_path"], pl, expt_str)
    else:
        px = 512
        py = 512
    ...
    # Load masks
    if not debug_bool:
        strc_mask = np.load(os.path.join(path_data["save_path"], 'strc_mask.npy'), allow_pickle=True).item()
    ...
    while (not debug_bool and counter_same < counter_same_thresh) or (debug_bool and data['frame'] <= debug_input.shape[1]):
        ...
        if not np.array_equal(im, last_frame) or debug_bool:
            ...
            if not debug_bool:
                ...
                outputSingleScan(s, ni_getimage)
                time.sleep(0.001)
                outputSingleScan(s, [0, 0, 0])
            
            if non_buffer_update_counter == 0:
                if not debug_bool:
                    unit_vals = obtain_roi(im, strc_mask)  # Function to obtain Rois values
                else:
                    ...
                    if buffer_update_counter == 0 and base_buffer_full:
                        ...
                        if back2baseline_flag:
                            if data['cursor'][data['frame']] <= back2base:
                                back2base_counter += 1
                            if back2base_ounter >= tset['back2base_frame_thresh']:
...
def clean_me_up(save_path, b_data, debug_bool, pl, data):
    ...
    save_filename = os.path.join(save_path, f'BMI_online_{now_str}.mat')
    
    # Save the data using scipy.io.savemat
    scipy.io.savemat(save_filename, {'data': data, 'bData': b_data})
